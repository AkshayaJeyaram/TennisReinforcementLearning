...
def train(env, num_updates = 700, rollout_length = 2048, initial_entropy_coef = 0.02):
    """
    ...
    for upd in range(1, num_updates + 1):
        ent_coef = initial_entropy_coef * max(0.0, 1 - upd/num_updates)
...
def evaluate(mappo,env, n_episodes = 5, alpha = 0.85, max_steps = 1000):
    ...
    for ep in range(1, n_episodes + 1): # because count begins from 0
        ...
        print(f"[SMOOTH EVAL] Ep {ep} | Scores: {scores} | Max: {scores.max():.3f}")
